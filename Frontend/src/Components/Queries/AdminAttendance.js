...
const AdminAttendance = () => {
  ...
  const fetchTodayStatus = useCallback(async () => {
    try {
      console.log('Fetching today status for admin:', user._id);
      const response = await axios.get(`/staff-attendance/today/${user._id}`);
      console.log('Admin today status response:', response.data);
      setTodayStatus(response.data);
    } catch (error) {
      console.error('Error fetching admin today status:', error);
  ...
  const fetchAttendanceHistory = useCallback(async () => {
    try {
      console.log('Fetching attendance history for admin:', user._id);
      ...
      console.log('Admin attendance history response:', response.data);
      setAttendanceHistory(response.data.attendance || []);
    } catch (error) {
      console.error('Error fetching admin attendance history:', error);
  ...
  useEffect(() => {
    console.log('AdminAttendance - User:', user);
    if (user?._id && user?.role === 'admin') {
      ...
      console.log('No admin user found, setting loading to false');
      setLoading(false);
    }
  }, [user, selectedMonth, selectedYear, fetchTodayStatus, fetchAttendanceHistory]);
  ...
  const calculateAttendancePercentage = () => {
    ...
    const presentDays = attendanceHistory.filter(a => ['present', 'late'].includes(a.status)).length;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Admin Attendance Dashboard</h1>
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {attendanceHistory.reduce((sum, record) => sum + (record.workingHours || 0), 0).toFixed(1)} hrs
              </span>
            </div>
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {attendanceHistory.length > 0 
                  ? (attendanceHistory.reduce((sum, record) => sum + (record.workingHours || 0), 0) / attendanceHistory.length).toFixed(1)
                  : 0} hrs
              </span>
            </div>
            <div className="flex justify-between">
              <span>Target:</span>
              <span className="font-semibold text-green-600">9 hrs/day</span>
            </div>
          </div>
        </div>
      </div>
