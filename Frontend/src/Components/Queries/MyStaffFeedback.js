  ...
  useEffect(() => {
    const fetchMyFeedbacks = async () => {
      try {
        ...
        const response = await axios.get(`/feedback/staff/${user._id}`);
    ...
    if (user && (user.role === "teacher" || user.role === "HOD") && user._id) {
      fetchMyFeedbacks();
  ...
  if (user.role !== "teacher" && user.role !== "HOD") {
    return (
      <div className="text-center p-8">
        ...
        <p className="text-gray-600">Only staff members can access this page.</p>
      </div>
    );
  }

  return (
    <main className="my-staff-feedback">
...
export default MyStaffFeedback;