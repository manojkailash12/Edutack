...
const InternalResultForm = () => {
  ...
  const downloadClassPDF = async () => {
    ...
    try {
      const response = await axios.get(`/internal/paper/${paper}/pdf`, {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', `Internal_Marks_Report_${paper}.pdf`);
        ...
        <form className="internal__body__form group">
          ...
          {internal.length && disabled ? (
            <div className="flex gap-4">
              ...
              <button
                type="button"
                className="mb-4 flex h-10 w-auto items-center gap-2 rounded-md border-[1.5px] border-solid border-blue-600 bg-blue-600 px-6 py-2 font-semibold tracking-wide text-white hover:bg-blue-700 focus:bg-blue-700"
