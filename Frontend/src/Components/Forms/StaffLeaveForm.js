...
const StaffLeaveForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("/leave/staff/submit", submitData);
  ...
  if (user.role !== "teacher" && user.role !== "HOD") {
    return (
      <div className="text-center p-8">
        ...
        <p className="text-gray-600">Only staff members can submit leave requests.</p>
      </div>
    );
  }

  return (
    <main className="staff-leave-form">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 max-w-2xl mx-auto">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="substitute" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            ...
            <input
              ...
              onChange={handleInputChange}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-violet-500 focus:border-violet-500 dark:bg-slate-700 dark:text-white"
              placeholder="Name of substitute teacher/staff member"
              required
            />
          </div>
