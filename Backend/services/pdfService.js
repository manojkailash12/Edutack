    ...
    if (true) { // Force table generation for debugging
      ...
      if (!subjects || subjects.length === 0) {
        ...
        doc.fontSize(9)
           .fillColor(colors.white)
           .font('Helvetica-Bold')
           .text('📝 No marks available yet. Contact your teacher to add marks.', leftMargin + 10, currentY + 10);
...
const generateAttendanceReportPDF = (res, data, filename) => {
  try {
    ...
    doc.fontSize(20)
       .fillColor(colors.white)
       .font('Helvetica-Bold')
       .text('📊 ATTENDANCE REPORT', 0, 25, { align: 'center' });
    ...
    if (data.students && data.students.length > 0) {
      ...
      doc.text(`Excellent (≥90%): ${excellentCount}`, 250, currentY + 35)
